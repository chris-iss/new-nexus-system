...
exports.handler = async (event, context) => {
  ...
  const extractCourseName = extractParameters.payload.course.name;
  ...
  if (getNetlifyKey === validationKey) {
    ...
    const updateThinkificLastActivityDateProperty = async (contactId, lastActivityDate) => {
      ...
      // Check if the course name is in the list
      if (courseList.includes(extractCourseName)) {
      ...
      // Make a PATCH request to update the HubSpot contact if a match was found
      if (lastActivity_DateProperty) {
    ...
    const hubspotSearchContact = async () => {
      ...
      const extractThinkificEmail = extractParameters.payload.user.email;
      const lastActivityDate = new Date(extractParameters.created_at);
